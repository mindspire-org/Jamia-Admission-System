...
import { tokensAPI, studentsAPI, gradesAPI } from "@/lib/api";
...
export default function TokenCounter() {
  ...
  useEffect(() => {
    ...
    if (editToken) {
      ...
      setFormData({
        ...
        cnic: editToken.cnic || "",
        passportNumber: editToken.passportNumber || "", // Load passport number
        bformNumber: (editToken as any).bformNumber || "", // Load B-Form number
        idType: editToken.idType || "cnic", // Load ID type
        contact: editToken.contact || "",
        class: editToken.class ? [editToken.class] : [],
        category: editToken.category || "",
        residency: "مقیم",
      });
      
      if (editToken.photoUrl) {
        setPhotoUrl(toAbsolutePhotoUrl(editToken.photoUrl));
      }
  ...
  const toAbsolutePhotoUrl = (url: string) => {
    if (!url) return "";
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    (async () => {
      ...
      if (editTokenId) {
        ...
        res = await tokensAPI.updateMy(editTokenId, {
          ...
          cnic: formData.cnic,
          passportNumber: formData.passportNumber, // Update passport number
          bformNumber: formData.bformNumber, // Update B-Form number
          idType: formData.idType, // Update ID type
        ...
        const serverPhotoUrl = toAbsolutePhotoUrl(s?.photoUrl || "");
        ...
        const newToken: TokenRecord = {
          ...
          testDate,
          resultDate,
          passportNumber: s?.passportNumber || formData.passportNumber, // Save passport number
          bformNumber: s?.bformNumber || formData.bformNumber, // Save B-Form number
          idType: (s?.idType || formData.idType) as "cnic" | "passport" | "bform", // Save ID type
        };
      ...
      searchTimeoutRef.current = setTimeout(async () => {
        try {
          ...
          if (data) {
            ...
            if (data.photoUrl) {
              setPhotoUrl(toAbsolutePhotoUrl(data.photoUrl));
            }
          ...
          <Card>
            ...
            <CardContent className="space-y-5">
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div className="space-y-2">
                  <Label htmlFor="class">جماعت (ایک سے زیادہ منتخب کریں)</Label>
                  ...
                  <div className="relative">
                    <button
                      type="button"
                      onClick={() => {
                        const el = document.getElementById('class-dropdown');
                    ...
                    >
                      <span className="text-muted-foreground">
                        {(formData.class as string[]).length > 0 
                          ? `${(formData.class as string[]).length} جماعتیں منتخب` 
                          : "جماعت منتخب کریں"}
                      </span>
                      <span className="text-muted-foreground">▼</span>
                    </button>
                    
                    <div id="class-dropdown" className="hidden absolute z-50 w-full mt-1 bg-background border rounded-md shadow-lg max-h-48 overflow-y-auto">
                      {classes.length > 0 ? (
                        classes.map((cls) => {
                        ...
                        <p className="text-muted-foreground text-center py-4">
                          کوئی جماعت دستیاب نہیں
                        </p>
                      )}
                    </div>
                  </div>
                  <p className="text-[10px] text-muted-foreground">
                    ایک یا ایک سے زیادہ جماعتیں منتخب کریں
                  </p>
                </div>
